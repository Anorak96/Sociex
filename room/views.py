...
from .forms import GroupForm, MessageForm, GroupUpdateForm
...
class RoomDetailView(generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        membs = group.members.order_by('group_admins')
        context['members'] = membs
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            instance.body = request.POST['message']
            instance.user = user
...
class GroupLeaveView(LoginRequiredMixin, generic.View):
    ...
    def post(self, request, pk):
        pk_ = request.POST.get('group_pk')
        group = Group.objects.get(pk=pk_)
        if self.request.user == group.created_by and group.mods.all():
            messages.info(request, 'You are the creator of this group and a Moderator.')
        else:         
            group.members.remove(self.request.user)
        return redirect(request.META.get('HTTP_REFERER'))
